...
S = 50; % number of value function segments
...
%% convert value function to 20 segments
...
vAvgt = zeros(S,T*Ts+1);
% average into hours
for h = 1:T*Ts
    vAvgt(:,h) = mean(vAvg(:,(h-1)/Ts + (1:1/Ts)),2);
end
...
for t = 1:T % start from the first day and move forwards
   vv = vAvgt(:,ceil(t*Ts)); % read the SoC value for this day
   [e, p] =  Arb_Value(lambda(t), vv, e, P, 1, eta, c, size(vAvgt,1));
   eS(t) = e; % record SoC
   pS(t) = p; % record Power
end
